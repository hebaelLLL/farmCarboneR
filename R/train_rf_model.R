...
train_rf_model <- function(train_data,
  ...
  predicteurs <- setdiff(
    names(train_data)[sapply(train_data, is.numeric)], target
  )
  ...
  if (!is.null(test_data) && target %in% names(test_data)) {
    # Garde uniquement les predicteurs presents dans test_data
    ...
    if (length(pred_test_vars) == length(predicteurs)) {
      ...
      warning("Certains predicteurs absents de test_data <e2><80><94> metriques test non calculees.")
    }
  }
